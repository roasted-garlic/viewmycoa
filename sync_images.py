...
PRODUCTION_URL = "https://your-replit-app.example.com"  # Update this!
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                logger.info(f"Successfully downloaded and verified: {save_path}")
                return True
            except Exception as img_error:
                logger.error(f"Downloaded file is not a valid image: {save_path} - {str(img_error)}")
    ...
    with app.app_context():
        ...
        for product in products:
            ...
            if product.product_image:
                ...
                product_dir = os.path.join('static', 'uploads', str(product_id))
                ...
                if not os.path.exists(save_path):
                    ...
                    logger.info(f"Downloading product image for product ID {product_id}: {image_url}")
                    if download_image(image_url, save_path):
                        success_count += 1
                else:
                    logger.info(f"Product image already exists locally: {save_path}")
            ...
            if product.label_image:
                ...
                if not os.path.exists(save_path):
                    ...
                    logger.info(f"Downloading label image for product ID {product_id}: {image_url}")
                    if download_image(image_url, save_path):
                        success_count += 1
                else:
                    logger.info(f"Label image already exists locally: {save_path}")
        
        logger.info(f"Image sync complete. Successfully downloaded {success_count} images.")
