...
def feature_request():
    ...
    feature_requests = models.FeatureRequest.query.order_by(models.FeatureRequest.created_at.desc()).all()
...
@app.route('/api/vote/<int:request_id>', methods=['POST'])
def vote(request_id):
    user_id = request.headers.get('X-Replit-User-Id')
    if not user_id:
        return jsonify({'error': 'Authentication required'}), 401
    ...
    existing_vote = models.Vote.query.filter_by(
    ...
    feature_request = models.FeatureRequest.query.get_or_404(request_id)
    return jsonify({
        'upvotes': feature_request.upvotes.filter_by(value=1).count(),
        'downvotes': feature_request.upvotes.filter_by(value=-1).count()
    })
